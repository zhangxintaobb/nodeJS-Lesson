...
//1、页面呈现：访问特定资源路径显示对应的页面
//2，博客列表：数据从服务端获取：jquery ajax从服务端获取数据
// $.get("",function(data){

// })
//3.http://localhost:8081/detail?chapderId
...
http.createServer(function(req,res){
    ...
    if(pathName=="/list/travedata"){
        traveData(res);
    }
    if(pathName.indexOf("css")>=0){
        showCss(res,filename);
    }
    ...
    if(pathName.indexOf("images")>=0){
        if(pathName.indexOf("img")>=0){
            ...
            showImages(res,filename);
        }
    }
    if(pathName == "/login/login_bg.jpg"){
        showJpg(res);
    }
    if(pathName=="/login/"){
        showLogin(res);
    ...
    if(pathName.indexOf("text")>=0){
        textpwd(req,res);
    }
    if(pathName=="/addChapter/"){
        addChapter(req,res);
    }
    if(pathName=="/add"&&req.method=="POST"){
        add(req,res);
    }
    if(pathName=="/listmanager/"){
        showManage(res);
    }
...
function add(req,res){
    ...
    req.on("end",function(){
        var Object=dataStr.split("&");
        var title=queryString.parse(Object[0]);
        var conten=queryString.parse(Object[1]);
        console.log(title);
        console.log(conten);
        option={
            "chapterList":0,
        ...
        option.chapterId=chapterList.length+1;
        option.chapterName=title.title;
        option.chapterContent=conten.content;
        chapterList.push(option);
...
function textpwd(req,res){
    var userList = [
        {username: "admin", pwd: "admin"}
    ]
    var urlObj=url.parse(req.url,true);
    console.log(urlObj.query.pwd);
    console.log(urlObj.query.username);
    if(urlObj.query.pwd==userList[0].pwd&&urlObj.query.username==userList[0].username){
        var Data=[{"a":"true"}];   
    }
    else{
        var Data=[{"b":"false"}];
...
function showManage(res){
    var ManagePath=path.join(__dirname,"list.html");
    var ManageContent=fs.readFileSync(ManagePath);
    res.writeHead(200,{"Content-tupe":"text/html"});
    res.write(ManageContent);
    res.end();
}

function showChapterData(res){
    var Data=[];
...
function showChapter(req,res){
    var urlObj=url.parse(req.url,true);
    id=urlObj.query.chapterId;
...
function traveData(res){
    var DataStr=JSON.stringify(chapterList);
    res.writeHead(200,{"Content-Type":"text/plain"});
    res.write(DataStr);
    res.end();
}
function showImages(res,filename){
    var imagesPath=path.join(__dirname,"/images/"+filename);
    var imagesContent=fs.readFileSync(imagesPath);
    res.writeHead(200,{"Content-tupe":"text/img"});
    res.write(imagesContent);
    res.end();
}
function showImg(res,filename){
    var imgPath=path.join(__dirname,"/images/img/"+filename);
    var imgContent=fs.readFileSync(imgPath);
    res.writeHead(200,{"Content-tupe":"text/img"});
    res.write(imgContent);
    res.end();
}
function showJs(res,filename){
    jsPath=path.join(__dirname,"/js/"+filename);
    jsContent=fs.readFileSync(jsPath);
...
function showCss(res,filename){
    cssPath=path.join(__dirname,"/css/"+filename);
    baseContent=fs.readFileSync(cssPath);
    res.writeHead(200,{"Content-Type":"text/css"});
    res.write(baseContent);
    res.end();
}